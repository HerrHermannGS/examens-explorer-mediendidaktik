...
def build_standalone_html_with_operators():
    ...
    # Baue Lookup-Table: {jahr}_{termin}_T{thema_nr} -> operatoren
    operatoren_map = {}
...
<body>
    ...
    <script>
        ...
        operatorenDetailliert.forEach(op => {{
            const key = `${{op.jahr}}_${{op.termin}}_T${{op.thema_nr}}`;
        ...
        function filterResults() {{
            ...
            filteredAufgaben = aufgaben.filter(aufgabe => {{
                ...
                return matchSearch && matchSemester && matchAFB;
            }});
    ...
    # Speichere HTML
    output_file = base_dir / 'dist' / 'examens_explorer_standalone.html'
